...
\begin{align*}
H(z)=\frac{\sum_{m=0}^{M}b_m z^{-k}}{\sum_{l=0}^{L}a_l z^{-l}}=\frac{B(z)}{A(z)}
...
\end{align}
La réponse impulsionnelle d'un filtre IIR s'obtient par récurrence en posant $x[n]=\delta[n]$.
Dans ce contexte, la technique de synthèse optimale consiste à minimiser l'erreur quadratique $\epsilon$ par rapport aux coefficients de la partie récursive et non-récursive du filtre.
\begin{proposition}[Technique de synthèse optimale]
Soit $\Omega=\{\widehat{a}_1,\cdots,\widehat{a}_L,\widehat{b}_0,\cdots,\widehat{b}_M \}$ les $L+M+1$ paramètres d'un filtre IIR.  La technique de synthèse optimale consiste à fixer $\Omega$ tel que 
\begin{align}
\widehat{\Omega}&=\arg \min_{\Omega \in \mathbb{R}^{L+M+1}} \sum_{n=0}^{N-1}(h_d[n]-h_\Omega[n])^2
\end{align}
où $h_\Omega[n]$ désigne la réponse impulsionnelle du filtre parametrée par $\Omega$. Cette réponse est donnée par
\begin{align*}
h_\Omega[n]&=\sum_{k=0}^{M} b_m \delta[n-k]-\sum_{l=1}^{L}a_l h_\Omega[n-l]
\end{align*}
\end{proposition}

Notons que la technique de synthèse optimale est généralement difficile à appliquer car elle nécessite de minimiser une fonction compliquée dans un espace de dimension $L+M+1$. Bien que cette minimisation puisse être réalisée en utilisant des algorithmes itératifs (fonction \texttt{minimize} en Python par exemple), la fonction de coût est généralement multimodale ce qui complexifie considérablement la recherche du maximum global.
