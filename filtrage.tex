...
\date{2016}
\begin{document}
\markboth{Introduction au Filtrage Numérique}{}
\maketitle
...
\begin{align}
y[n]=\sum_{k=0}^{M}b_k x[n-k]-\sum_{l=1}^{L}a_l y[n-l],\label{filtre1}
\end{align}
où les coefficients réels $b_k$ ($k=0,\cdots,M$) et $a_k$ ($k=1,\cdots,L$) correspondent respectivement aux coefficients de la partie non récursive et récursive du filtre. Notons que le coefficient multipliant $y[n]$ est implicitement égal à $a_0=1$. L'ordre du filtre est défini comme étant la plus grande valeur entre $N$ et $L$. 

La réponse impulsionnelle d'un filtre décrit par une équation aux différences s'obtient en posant $x[n]=\delta[0]$ dans l'équation (\ref{filtre1}). En fonction des coefficients $a_k$, il est alors possible de définir deux grandes catégories de filtres:
\begin{itemize}
...
\begin{itemize}
\item Connaissant la valeur des coefficients $b_k$ et $a_l$, est-il possible de comprendre l'effet d'un filtre sur une entrée quelconque $x[n]$  ? 
\item Est-il possible de déterminer les coefficients $b_k$ et $a_l$ d'un filtre à partir d'un ensemble de spécifications (fréquentielles ou temporelles) ?
\end{itemize}
...
\begin{align*}
y[n]&=0.065x[n]+0.135 x[n-1]+0.065x[n-2])\\
...
La propriété liée à la convolution montre que la fonction de transfert d'un filtre s'exprime également sous la forme $H(z)=Y(z)/X(z)$. Pour un filtre décrit par une équation aux différences, cette propriété permet d'exprimer la fonction de transfert du filtre en fonction des coefficients des parties récursive $a_l$ et non-recursive $b_k$ du filtre.
...
\begin{align*}
H(z)=\frac{\sum_{k=0}^{M}b_k z^{-k}}{\sum_{l=0}^{L}a_l z^{-l}}=\frac{B(z)}{A(z)}
...
\begin{align}
H(z)=\frac{0.065+0.135 z^{-1}+0.065z^{-2}}{1-1.143z^{-1}+0.413z^{-2}}.
\end{align}
...
\begin{align*}
H(z)=\left(\frac{b_0}{a_0}\right)\frac{\prod_{k=1}^{M}(1-c_k z^{-1})}{\prod_{l=1}^{L}(1-d_l z^{-1})}
\end{align*}
où les valeurs $c_k$ et $d_l$ correspondent respectivement aux zéros et aux pôles de la fonction de transfert.
\end{definition}
...
\caption{Filtre1: Cartographie des Pôles et des Zéros}
\label{fig_carte_pz}
\end{subfigure}%
\begin{subfigure}[b]{0.5\textwidth}
\centering
...
\end{axis}
\end{tikzpicture}
\caption{Filtre1: Réponse Impulsionnelle}
\end{subfigure}
\vskip 1em
\begin{subfigure}[b]{0.5\textwidth}
\centering
...
\addplot [dashed,domain=0:2*pi,samples=50]({cos(deg(x))},{sin(deg(x))});
\end{axis}\end{tikzpicture}
\caption{Filtre2: Cartographie des Pôles et des Zéros}
\end{subfigure}%
\begin{subfigure}[b]{0.5\textwidth}
\centering
...
\end{axis}
\end{tikzpicture}
\caption{Filtre2: Réponse Impulsionnelle}
\end{subfigure}
...
Pour un filtre LTI, $Y(e^{j\omega})=H(e^{j\omega})X(e^{j\omega})$ c-a-d que la transformée de Fourier de la sortie s'obtient en multipliant la transformée de Fourier de la réponse impulsionnelle par la transformée de Fourier de l'entrée. La transformée de Fourier de la réponse impulsionnelle est appelée réponse fréquentielle du filtre.
...
Pour les filtres décrits par une équation aux différences, la réponse fréquentielle peut s'exprimer en fonction de la transformée de Fourier des coefficients $a_l$ et $b_k$ du filtre.
\begin{proposition}[Réponse Frequentielle]
La réponse fréquentielle d'un filtre s'exprime sous la forme
\begin{align*}
H(e^{j\omega})\triangleq \frac{Y(e^{j\omega})}{X(e^{j\omega})}=\frac{\sum_{k=0}^{M}b_k e^{-j\omega k}}{\sum_{l=0}^{N}a_l e^{-j\omega l}}=\frac{B(e^{j\omega})}{A(e^{j\omega})}
\end{align*}
\end{proposition}
...
Phase Linéaire & Oui si  $b_k=b_{M-k}$& Non\\
Ordre & Elevé & Faible \\
\hline
\end{tabular}
...
Dans le domaine analogique, un filtre est décrit par une équation différentielle. Pour numériser un filtre analogique, une approche possible consiste à utiliser une technique d'intégration numérique pour résoudre cette équation différentielle. En utilisant la méthode des trapèzes, nous obtenons une technique de synthèse nommée transformée bilinéaire. En pratique, la transformée bilinéaire s'obtient simplement en réalisant la substitution $s=c\frac{1-z^{-1}}{1+z^{-1}}$, où $c$ correspond à un paramètre libre du filtre. 
...
\end{align}
\end{proposition}
Un point appartenant à l'axe $s=j\omega$ dans le plan de Laplace est déplacé en un point localisé sur le cercle de rayon unité $e^{j\omega}$ dans le domaine en $\mathcal{Z}$. De plus, il est démontré dans l'annexe \ref{ap_tf}, qu'après transformation, la pulsation $\omega_a$ dans le domaine continue est déplacée à la pulsation $\omega=\frac{2}{T_e}\textrm{atan}(\omega_a/c)$ dans le domaine numérique. Ce \textit{warping frequentiel} non-linéaire est illustré par la figure \ref{fig_warp} pour $c=T_e=1$. 
...
\section{Synthèse par modèle de réponse impulsionnelle}
\label{s_design_ri}
Un filtre numérique est décrit par en ensemble de coefficients $b_k$ et $a_k$. Pour synthétiser un filtre, une technique possible consiste à minimiser un critère d'erreur entre la réponse attendue (impulsionnelle ou fréquentielle) et la réponse du filtre. Un critère d'erreur couramment utilisé est l'erreur quadratique. Dans cette partie, nous allons voir comment fixer les coefficients $b_k$ et $a_l$ afin de minimiser l'erreur quadratique entre la réponse impulsionnelle attendue et la réponse impulsionnelle du filtre c-a-d
\begin{align}
\epsilon&=\sum_{n=-\infty}^{\infty}(h_d[n]-h[n])^2\label{eq_ref_h}
\end{align}
Notons que la réponse impulsionnelle désirée $h_d[n]$ est parfois donnée implicitement via la réponse fréquentielle, $H_d(e^{j\omega})$. Dans le cas, il est nécessaire de calculer $h_d[n]$ via l'expresssion (\ref{f_to_h}).

Pour illustrer cette section, nous allons synthétiser le filtre suivant.
\begin{exemple}[Filtre 3]
Le filtre 3 est un filtre passe-bas en mur de brique (pente infinie) comportant une phase linéaire. La réponse fréquentielle de ce filtre est donnée par $H_d(e^{j\omega})=|H_d(e^{j\omega})|e^{-j\omega M/2}$ où
\begin{align}
...
\end{exemple}
La réponse impulsionnelle du filtre 3 s'obtient en utilisant l'équation (\ref{f_to_h}). Après quelques calculs, nous obtenons
\begin{align}
h_d[n]&=\frac{\sin\left(\omega_a T_e (n-M/2)\right)}{\pi (n-M/2)}
\end{align}
La réponse impulsionnelle de ce filtre est présentée dans la figure \ref{fig_fir_ri} pour $M=100$ échantillons.
\begin{figure}[t]
\centering
\begin{tikzpicture}
...
Mathématiquement, un filtre FIR est modélisé par la relation
\begin{align}
...
\end{array}\right.
\end{align}
Dans ce contexte, la minimisation de l'erreur quadratique $\epsilon$ dans (\ref{eq_ref_h}) s'obtient facilement en imposant l'égalité $b_k=h_d[k]$ dans l'intervalle $0\le n \le M$. Cette technique est couramment appelée technique de synthèse par fenêtrage car la réponse impulsionnelle obtenue correspond à la réponse impulsionnelle attendue multipliée par une fenêtre rectangulaire. Notons qu'il est possible de généraliser cette technique en remplaçant la fenêtre rectangulaire par une fenêtre de pondération $w[k]$. 
\begin{proposition}[Technique de synthèse par fenêtrage]
La technique de synthèse par fenêtrage s'obtient en fixant 
\begin{align}
\widehat{b}_k=\left\{\begin{array}{cl}w[k]h_d[k]&\textrm{ pour }0\le n \le M\\
0& \textrm{ ailleurs}
\end{array}\right.
\end{align}
où $w[k]$ est une fenêtre de pondération.
\end{proposition}
...
\end{align}
Pour $n\ge 0$, la réponse impulsionnelle d'un filtre IIR est donnée par
\begin{align*}
h[n]&=\sum_{k=0}^{M} b_k \delta[n-k]-\sum_{l=1}^{L}a_l h[n-l]
\end{align*}
La minimisation directe de l'erreur quadratique $\epsilon$ dans (\ref{eq_ref_h}) est généralement difficile. D'une part, le calcul de l'erreur nécessite de sommer une infinité de termes. D'autre part, le problème de minimisation est non-linéaire par rapport aux coefficients $a_k$. Pour résoudre ce problème, une technique alternative consiste à minimiser, au lieu de l'erreur $\epsilon$, l'erreur quadratique \textit{tronquée et filtrée} définie par
\begin{align}
\epsilon_f&=\sum_{n=0}^{N-1}(h_{df}[n]-b[n])^2\label{eq_ref_h2}
\end{align}
où $h_{df}[n]=\sum_{l=0}^{L}a_l h_d[n-l]$ et $a_0=1$. Lorsque $N$ est suffisamment grand, cette technique permet d'obtenir une réponse impulsionnelle proche de la réponse impulsionnelle attendue. Cette approche est couramment appelée méthode de Prony en référence à son inventeur. En utilisant cette approche, il est démontré en annexe \ref{ap_prony} que les coefficients $a_l$ et $b_k$ s'obtiennent de la manière suivante.
...
En appliquant la transformée en $\mathcal{Z}$ de part et d'autre de l'équation (\ref{filtre1}) et en appliquant la propriété de Linéarité de la transformée en $\mathcal{Z}$, nous obtenons
 \begin{align*}
 a_0Y(z)&=\sum_{k=0}^{M}b_k z^{-k} X(z) -\sum_{l=1}^{L}a_l z^{-l}Y(z).
  \end{align*}
 En factorisant par $Y(z)$ et $X(z)$, nous avons
  \begin{align*}
 Y(z)\left(\sum_{l=0}^{L}a_l z^{-l}\right)&=X(z)\left(\sum_{k=0}^{M}b_k z^{-k}\right)
   \end{align*}
 La fonction de transfert $H(z)=Y(z)/X(z)$ s'exprime alors sous la forme
  \begin{align*}
 \frac{Y(z)}{X(z)}&=\frac{\sum_{k=0}^{M}b_k z^{-k}}{\sum_{l=0}^{L}a_l z^{-l}}.
 \end{align*}
...
\begin{align*}
&Y(z)=\sum_{k=0}^{p}b_k X(z)z^{-k}-\sum_{l=1}^{q}a_l Y(z)z^{-l}\\
&Y(z)\left(\sum_{l=0}^{q}a_l z^{-l}\right)=X(z) \left(\sum_{k=0}^{p}b_k z^{-k}\right)
\end{align*}
La fonction de transfert est alors égal à 

\begin{align*}
H(z)\triangleq \frac{Y(z)}{X(z)}=\frac{\sum_{k=0}^{p}b_k z^{-k}}{\sum_{l=0}^{q}a_l z^{-l}}
\end{align*}
...
\label{ap_tf}
La transformée bilinéaire s'obtient en réalisation a substitution
\begin{align}
...
\end{align}
Dans le domaine de Laplace, le domaine fréquentiel s'obtient en posant $s=j\omega_a$. En posant $s=j\omega_a$ dans l'équation (\ref{ap_eq_bil_0}), nous obtenons $|z|=1$. L'axe des imaginaires purs dans le domaine de Laplace est donc mappé en un cercle de rayon unité dans le domaine en $\mathcal{Z}$. De plus, il en vient que 
\begin{align}
...
&=jc\tan(\omega T_e/2)
\end{align}
La pulsation $\omega_a$ dans le domaine continue est donc mappé à la pulsation $c\tan(\omega T_e/2)$ dans le domaine numérique. De même la pulsation numérique $\omega$ correspond à la pulsation $\frac{2}{T_e}\textrm{atan}(\omega_a /c)$ dans le domaine analogique.
...
\end{align}
Comme $b_k=0$ pour tout $k>M$, la somme peut se décomposer en deux parties:
\begin{align}
